function [states] = AM_A_PV(info)
    ...
    if do_plot_flag
        ...
        if save_movie_flag
            ...
            movie_name = sprintf("AM_%s_movie_seed_%d", Att_motion, seed);
    ...
    Att_v = 0.000001*ones(Att_num,1);           % used for attacker acceleration (see end)
    ...
    while sum(Def_alive)>Def_final_fraction*Def_num
        ...
        % Attacker Accleration (optional)
        vecnorm_vals = vecnorm(Att_vel, 2, 2);                      % Calculate the norms of the velocity vectors
        zero_norm_idx = vecnorm_vals == 0;                          % Find indices where the norm is zero
        % Att_accel = zeros(size(Att_vel));                           % Initialize Def_accel: equals zero where the norm is zero
        non_zero_norm_idx = ~zero_norm_idx;                         % Compute acceleration only for non-zero norms
        Att_accel(non_zero_norm_idx,:) = Att_accel_max * Att_vel(non_zero_norm_idx,:) ./ vecnorm_vals(non_zero_norm_idx);
        Att_accel = Att_accel - Att_vel / Att_accel_steps;          %  A = max cmd - already attained
        Att_vel = Att_vel + Att_accel;                              % Vx = vprev + accel
        over_idx = find(vecnorm(Att_vel, 2, 2) > Att_vel_max);      % if V > Vmax, set V = Vmax

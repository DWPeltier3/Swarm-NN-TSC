function [states] = HVUP_A_PV(info)
    ...
    % HVU_theta   = pi/4;                             % fixed initial heading [rad]
    HVU_theta   = pi/2*rand;                        % random initial heading [rad]
    ...
    while sum(Def_alive)>Def_final_fraction*Def_num
        ...
        %% Distances between each attacker and HVU
        hvu_dist = vecnorm(Att_pos-HVU_pos,2,2);
        
        %% Each attacker kills HVU or moves towards it
        iter=1;
        while iter <= Att_num % for each attacker
            if hvu_dist(iter) < Att_kill_range % kill HVU if able
                % disp("HVU killed!")
                return
            else %if can't kill HVU, move towards it
                xdiff=HVU_pos(1)-Att_pos(iter,1);
                ydiff=HVU_pos(2)-Att_pos(iter,2);
                c2=HVU_vel(1)^2+HVU_vel(2)^2-Att_vel_max^2;
                c3=2*xdiff*HVU_vel(1)+2*ydiff*HVU_vel(2);
                c4=xdiff^2+ydiff^2;
                ...
                if discriminant < 0
                    ...
                    ts=roots([c2 c3 c4]);
                    ts=max(ts);
                    Att_Velocity_vect(iter,1)=((xdiff+HVU_vel(1)*ts))/ts;
                    Att_Velocity_vect(iter,2)=((ydiff+HVU_vel(2)*ts))/ts;
        ...
        HVU_pos = HVU_pos+HVU_vel;                       %  P = Pprev + V

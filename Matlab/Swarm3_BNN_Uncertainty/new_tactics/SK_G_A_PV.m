function [states] = SK_G_A_PV(info)
    ...
    if do_plot_flag
        ...
        if save_movie_flag
            ...
            movie_name = sprintf("SK_1G_movie_seed_%d", seed);
    ...
    while sum(Def_alive)>Def_final_fraction*Def_num
        ...
        for att = 1:Att_num
            ...
            [min_val, closestDef] = min(Dist(att,:));
            if (min_val < Att_kill_range) && (Def_alive(closestDef) == 1)
                % Kill the defender if in range
                Def_pos(closestDef,:) = NaN;
                Dist(:,closestDef) = NaN;
                Def_vel(closestDef,:) = 0;
                Def_alive(closestDef) = 0;
                target_vec = [0, 0]; % no target if already killed
            else
                if Def_alive(closestDef) == 1
                    target_vec = Def_pos(closestDef,:) - Att_pos(att,:);
                    if norm(target_vec) > 0
                        target_vec = target_vec / norm(target_vec); % unit vector
                    end
                else
                    target_vec = [0, 0];
                end
            ...
            for other_att = 1:Att_num
                if att ~= other_att
                    ...
                    if (d < min_dist) && (d > 0)
                        ...
                        repel_vec = repel_vec + (min_dist - d) * v_perp; % Weight repulsion by (min_dist - d)
            ...
            % Apply a gain to the repulsion contribution
            ...
            % Set the final acceleration (direction times maximum acceleration)
        ...
        %% NOTE: Actual "accel_steps" (i.e. 10 steps to full speed) causes issues with attackers orbiting around defenders (can't capture)

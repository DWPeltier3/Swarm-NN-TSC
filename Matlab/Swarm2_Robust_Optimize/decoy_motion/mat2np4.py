...
parser.add_argument('--swarm_size', type=int,
                    help='swarm size')
parser.add_argument('--decoy_motion', type=str,
                    help='decoy motion')
# Parse the arguments
args = parser.parse_args()
# swarm_size = args.swarm_size
swarm_size = 10
input_num_runs = 10000*4
decoy_motion = args.decoy_motion # only used when passed one decoy motion at a time
# Define the types of decoy motion
...
print(f'MATLAB .mat --> Numpy Converter\n'
      ...
      f'Number of Runs: {input_num_runs}\n' # Second experiment with 10k runs per tactic per motion
      f'Velocity = random 0.05-0.4\n')


## IMPORT MATLAB matrix as np array
# data_folder=f"/home/donald.peltier/swarm/data/mat_files/{swarm_size}v{swarm_size}_{decoy_motion}/" # folder that contains .mat files; varying decoy motion
data_folder=f"/home/donald.peltier/swarm/data/mat_files/{swarm_size}v{swarm_size}_{decoy_motion}_r{input_num_runs}/" # Second experiment with 10k runs per tactic per motion
...
# Calculate the min, mean, variance, and median
...
plt.axvline(min_time, color='red', linestyle='dashed', linewidth=2)
...
plt.text(median_value+2, plt.ylim()[1]*0.5, f'median={median_value:.0f}', color='red')
plt.text(min_time+2, plt.ylim()[1]*0.3, f'min={min_time:.0f}', color='red')
plt.xlabel('Time Steps')
plt.ylabel('Frequency')
plt.title('Histogram of Engagement Total Time Steps')
# Show the plot
plt.savefig(data_folder + f"Time_histogram_{swarm_size}v{swarm_size}_{decoy_motion}.png")
...
# filename=f'data_{swarm_size}v{swarm_size}_{decoy_motion}.npz' # First experiment with 1200 runs per tactic per motion
filename=f'data_{swarm_size}v{swarm_size}_{decoy_motion}_r{input_num_runs}.npz' # Second experiment with 10k runs per tactic per motion
...
# filename=f'mean_var_{swarm_size}v{swarm_size}_{decoy_motion}.npz' # First experiment with 1200 runs per tactic per motion
filename=f'mean_var_{swarm_size}v{swarm_size}_{decoy_motion}_r{input_num_runs}.npz' # Second experiment with 10k runs per tactic per motion
...
# filename=f'mean_var_{swarm_size}v{swarm_size}_{decoy_motion}.mat'
# full_path = os.path.join(data_path, mean_var_folder, filename)

function [states] = GreedyPro_D_PV(attacker, defender, defender_v, do_plot, kill_pro, seed, accel, kill_range, rand_start, Def_motion)
    ...
    %elseif decoy_motion == "split"
        %theta1 = 135 * pi/180;  % group 1 angle in radians
        %theta2 = theta1+pi;     % group 2 angle in radians
        %N_group=round(N_def/2); % number per group
        %vel(1:N_group,1) = v * cos(theta1);  % x component of velocity for group 1
        %vel(1:N_group,2) = v * sin(theta1);  % y component of velocity for group 1
        %vel(N_group+1:end,1) = v * cos(theta2);  % x component of velocity group 2
        %vel(N_group+1:end,2) = v * sin(theta2);  % y component of velocity group 2

function datagenarray(decoy_motion)
    ...
    % seedrange=1200; % original # of samples (runs) to collect (per algorithm)
    seedrange=10000; % # of samples (runs) to collect (per algorithm)
    num_runs = 4*seedrange;
    ...
    for seed=1:seedrange
        ...
        %% GREEDY (Pursuit)
        g=Greedy_D_PV(N_attacker,N_defender,Defender_v_max,do_plot,final_fraction,seed,accel,kill_range,rand_start,decoy_motion);

        %% GREEDY ProNav
        gp=GreedyPro_D_PV(N_attacker,N_defender,Defender_v_max,do_plot,final_fraction,seed,accel,kill_range,rand_start,decoy_motion);

        %% Auction (Pursuit)
        a=Auction_D_PV(N_attacker,N_defender,Defender_v_max,do_plot,final_fraction,seed,accel,kill_range,rand_start,decoy_motion);

        %% Auction ProNav
        ap=AuctionPro_D_PV(N_attacker,N_defender,Defender_v_max,do_plot,final_fraction,seed,accel,kill_range,rand_start,decoy_motion);
    ...
    %% Save data matricies
    if savemat
        % Dynamic folder path based on inputs
        folderName = sprintf('%dv%d_%s_r%d', N_attacker, N_defender, decoy_motion, num_runs);
        folderPath = ['/home/donald.peltier/swarm/data/mat_files/' folderName '/']; % Concatenate folder name with base path
        ...
        sprintf('saved mat files for %s motion to %s', decoy_motion, folderPath);
    end
end
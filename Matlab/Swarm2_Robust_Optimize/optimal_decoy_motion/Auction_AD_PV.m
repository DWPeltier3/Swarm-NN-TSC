function [states] = Auction_AD_PV(info)
    ...
    if do_plot_flag      % Initialize VideoWriter object to save engagement movie
        figure(10);
        if save_movie_flag
    ...
    while sum(Def_alive)>Def_final_fraction*Def_num
        ...
        Dist=zeros(Att_num,Def_num); % distance matrix (row=defender, col=attacker)
        iter=1; %init counter

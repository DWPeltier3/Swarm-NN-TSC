function [states] = OBS_AP_A_PV(info)
    ...
    Att_obs_dist        = info.Att_obs_dist;  % New variable for safe distance
    repulsion_strength  = info.repulsion_strength;  % New variable for repulsion strength
    ...
    while sum(Def_alive)>Def_final_fraction*Def_num
        ...
        if do_plot_flag
            ...
            % plot(Att_pos(1,1),Att_pos(1,2),'g.','MarkerSize',16)
        ...
        Dist_check=dists;
        for att = 1:Att_num
            % Find defenders within the observation distance
            close_defenders = Def_pos(dists(att,:) < Att_obs_dist, :);
            if isempty(close_defenders)
                [~, def] = min(Dist_check(att,:)); % find closest defender
                target_num(att)=def;            % assign to attacker
                Dist_check(att,:)=NaN;          % prevent double ups
                Dist_check(:,def)=NaN;
                %calculate intercept and point assigned attacker acceleration vector to this intercept
                xdiff=Def_pos(def,1)-Att_pos(att,1);
                ydiff=Def_pos(def,2)-Att_pos(att,2);
                c2=Def_vel(def,1)^2+Def_vel(def,2)^2-Att_vel_max^2;
                c3=2*xdiff*Def_vel(def,1)+2*ydiff*Def_vel(def,2);
                c4=xdiff^2+ydiff^2;
                ...
                if discriminant < 0
                    ...
                    ts=roots([c2 c3 c4]);
                    ts=max(ts);
                    Att_Accel_vect(att,1)=((xdiff+Def_vel(def,1)*ts))/ts;
                    Att_Accel_vect(att,2)=((ydiff+Def_vel(def,2)*ts))/ts;
                    vec=Att_Accel_vect(att,:);
                end
                vec_norm = norm(vec);
                if vec_norm ~= 0
                ...
                approaching_defenders = Def_pos(dists(att,:) < 2*Att_obs_dist, :);
                if ~isempty(approaching_defenders)
                    Att_accel(att,:) = 0.5*Att_accel(att,:);
                end
            else
                % Calculate average repulsive vector
                repulsion_vec = zeros(1, 2);
                for j = 1:size(close_defenders, 1)
                    repulsion_dir = (Att_pos(att,:) - close_defenders(j,:)) / norm(Att_pos(att,:) - close_defenders(j,:));
                    repulsion_vec = repulsion_vec + repulsion_dir;
                end
                repulsion_vec = repulsion_vec / size(close_defenders, 1);
                Att_accel(att,:) = repulsion_strength * repulsion_vec;
            end
        end
...
%{
        ...
        while iteri<=Att_num && iteri+totalkilled<=Def_num
            maxMatrix=max(Disto_check(:)); %scalar (max defender distance from origin)
            ...
            [iter,iter2] = find(Dist_check==minMatrix); %find attacker and defender that meets criteria above
            Dist_check(iter,:)=NaN;
            ...
            if Def_alive(iter2,1)==1
                ...
                Att_accel(iter,:) = Att_accel_max * vec;
                % Att_accel(iter, 2) = Att_accel_max * vec(2);
                if Dist(iter,iter2)<Att_obs_dist % attacker inside observation distance
                    Att_accel(iter) = - Att_accel(iter);
                end
            end
            iteri=iteri+1;
        end
%}
        ...
        HVU_pos = HVU_pos+HVU_vel;                                      %  Px = xprev + xvel

...
# Add arguments
parser.add_argument('--swarm_size', type=int, help='swarm size')
...
# swarm_size = args.swarm_size # swarm_size = 10
# decoy_motions=("straight" "perpendicular" "split" "semi" "r4800_c4_a10") # types of decoy motion
...
data_folder=f"/home/donald.peltier/swarm/data/mat_files/{num_att}v{num_def}_{decoy_motion}/" # folder that contains .mat files
...
mode_value = statistics.mode(time)
...
plt.text(mean_time+2, plt.ylim()[1]*0.8, f'variance={variance_value:.0f}', color='red')
plt.text(mode_value+2, plt.ylim()[1]*0.5, f'mode={mode_value:.0f}', color='red')
plt.text(min_time+2, plt.ylim()[1]*0.3, f'min={min_time:.0f}', color='red')
plt.xlabel('Time Steps')
plt.ylabel('Frequency')
plt.title('Histogram of Engagement Total Time Steps')
# Show the plot
plt.savefig(data_folder + f"Time_histogram_{num_att}v{num_def}_{decoy_motion}.png")
...
filename=f'data_{num_att}v{num_def}_{decoy_motion}.npz'
...
filename=f'mean_var_{num_att}v{num_def}_{decoy_motion}.npz'
...
# filename=f'mean_var_{num_att}v{num_def}_{decoy_motion}.mat'
# full_path = os.path.join(data_path, mean_var_folder, filename)

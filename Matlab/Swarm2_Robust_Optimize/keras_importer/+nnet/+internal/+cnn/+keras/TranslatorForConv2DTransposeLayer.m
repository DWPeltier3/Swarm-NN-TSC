classdef TranslatorForConv2DTransposeLayer < nnet.internal.cnn.keras.LayerTranslator
...
% Copyright 2017-2019 The MathWorks, Inc.
    ...
    methods
        function NNTLayers = translate(this, LSpec, TranslateWeights, TranslateTrainingParams, UserImageInputSize)
            ...
            if TranslateWeights
                % keras shape: [numberofFiltersx1], Maltab size 1-by-1-by-NumFilters matrix.
                if UseBias
                    verifyWeights(LSpec, 'bias');
                    Conv2dTransposed.Bias = single(reshape(LSpec.Weights.bias, [1,1,NumFilters]));
                else
                    Conv2dTransposed.Bias = zeros(1,1,NumFilters, 'single');
                end
                % format of LayerSpec.Weights.kernel: NumChannels-NumFilters-ncol-nRow:
                %                                                       C x K x W x H
                % MATLAB: FilterSize(1)-by-FilterSize(2)-by-NumFilters-by-NumChannels:
                %                                                       H x W x K x C
                verifyWeights(LSpec, 'kernel');
                Conv2dTransposed.Weights = single(permute(LSpec.Weights.kernel, [4,3,2,1]));
            end
            NNTLayers = {Conv2dTransposed};
            NNTLayers = maybeAppendActivationLayer(LSpec, NNTLayers);
        end
        
        function [tf, Message] = canSupportSettings(this, LSpec)
            if ~(isequal(kerasField(LSpec, 'padding'), 'valid') || ...
                    isequal(kerasField(LSpec, 'padding'), 'same'))
                tf = false;

classdef ConverterForMaxPooling3DLayer < nnet.internal.cnn.tf_exporter.LayerConverter
        ...
        function convertedLayer = toTensorflow(this)
            ...
            if isequal(this.Layer.PaddingMode, 'same')
                ...
                convertedLayer.layerCode = kerasCodeLine(this, this.InputTensorName, this.OutputTensorName,...
                    "layers.MaxPool3D", "pool_size=%s, strides=%s, padding=""same""", {pool_size, strides},...
                    "", useTimeDistributed);
            elseif all(PaddingSize(:) == 0)
                % Generate only a maxpool line with valid padding
                convertedLayer.layerCode = kerasCodeLine(this, this.InputTensorName, this.OutputTensorName,...
                    "layers.MaxPool3D", "pool_size=%s, strides=%s", {pool_size, strides},...
                    "", useTimeDistributed);
            else
                % This handles 'same' padding and manual padding. First pad
                % with zeros, then pool with valid padding. 3D padding
                % layout is a 2-by-3 matrix [t l f;b r k]
                t = PaddingSize(1,1);
                b = PaddingSize(2,1);
                l = PaddingSize(1,2);
                r = PaddingSize(2,2);
                ...
                convertedLayer.layerCode = [
                    ...
                    "layers.ZeroPadding3D", "padding=((%d,%d),(%d,%d),(%d,%d))", {t, b, l, r, f, k},...
                    "", useTimeDistributed)

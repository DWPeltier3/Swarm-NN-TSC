classdef ConverterForGlobalAveragePooling1DLayer < nnet.internal.cnn.tf_exporter.LayerConverter
        ...
        function convertedLayer = toTensorflow(this)
            ...
            switch this.InputFormat
                case {"CBT", "CT"}
                    ...
                    % case
                    convertedLayer.layerCode = kerasCodeLine(this, this.InputTensorName, this.OutputTensorName, "layers.GlobalAveragePooling1D",...
                ...
                case {"SCB", "SC"}
                    ...
                    convertedLayer.layerCode = kerasCodeLine(this, this.InputTensorName, this.OutputTensorName, "layers.GlobalAveragePooling1D",...
                        "keepdims=True", {}, "", false);
                case {"SCBT", "SCT"}
                    % In Keras these are BTSC. To implement globalAvgPool1D
                    % over S using AveragePool2D, we make the poolsize in S
                    % full-size, and the poolsize in T 1.
                    convertedLayer.layerCode = [
                        sprintf("BTSC = %s.shape", this.InputTensorName)
                        sprintf("%s = layers.AveragePooling2D(pool_size=(1,BTSC[2]))(%s)", this.OutputTensorName, this.InputTensorName)
                        ];
                otherwise
